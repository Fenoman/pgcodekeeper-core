...
- Добавлено чтение конструкций, в которые PostgreSQL сам разворачивает вызовы функций: `AT LOCAL` (PostgreSQL 17), `IS [NOT] NORMALIZED` и `NORMALIZE()` (PostgreSQL 13), `SYSTEM_USER` (PostgreSQL 16). Представление, значение по умолчанию или ограничение, написанное обычным вызовом, выходит из базы именно в таком виде, поэтому раньше инструмент выгружал в проект файл, который сам же не мог прочитать, а оператор пропадал из модели вместе со всем, что от него зависит.
- Добавлена настройка глубины сравнения. Сравнению, которому нужно только дерево различий, больше не требуется полный анализ обеих сторон, и оно завершается заметно быстрее; сравнение, которое строит скрипт миграции, по-прежнему получает полную модель.
- Добавлено повторное использование разобранных моделей проекта между запусками. Проект, не изменившийся с предыдущего сравнения, восстанавливается из постоянного индекса вместо повторного разбора и анализа.
- Добавлены постоянные кэши тел функций и строк системного каталога PostgreSQL: повторное сравнение с той же базой больше не перечитывает то, что не менялось.
- Добавлен файл-фильтр, решающий, какие файлы верхнего уровня проекта участвуют в загрузке. Он применяется до разбора, поэтому отклонённые файлы не стоят ничего.
- Добавлено точное исключение схем при загрузке проекта — до разбора файлов. Оператор, который разрешается в исключённую схему, отбрасывается намеренно и не попадает в ошибки проекта.
- Правила игнорирования распространены на вложенные объекты и на отдельные колонки. Правило может скрыть колонку, ограничение, индекс или триггер, не скрывая объект-владелец.
- Добавлена возможность перечислить колонки в правиле игнорирования: перечисленные колонки не участвуют в сравнении, а сама таблица участвует.
- Добавлен отчёт о том, сколько объектов и колонок правила игнорирования исключили из сравнения.
- Добавлено отображение хода анализа при загрузке проекта, чтобы долгая загрузка не выглядела зависшей, и отдельный замер времени загрузки для исходной и целевой стороны.
- Добавлена настройка, оставляющая размер кэша последовательности таким, какой он в базе.
- Добавлена настройка, записывающая `ALTER TABLE` без `ONLY`, чтобы изменение доходило до всего дерева наследования или секционирования.
- Добавлена настройка, оставляющая целевой объём статистики колонки таким, какой он в базе.
- Добавлено сохранение значений, которыми владеет проект, при применении к нему состояния базы.
- Добавлено предупреждение, когда миграция отличается от проекта только регистром имени колонки: применение такой миграции теряет данные колонки.
- Добавлены пометки в дереве различий о том, что миграция перенести не может, — например, о смене сортировки или о значении, которое сравнение не учитывает.
- Добавлен исходный коммит сборки в манифест бандла: два бандла с одной и той же версией теперь различимы, а упаковка отвергает бандл, манифест которого не соответствует исходникам, из которых он собран.
- Расширен публичный API: сравнение можно загрузить один раз и использовать повторно, его графы зависимостей — разделять между запусками, а методы построения дерева различий и скрипта миграции принимают уже загруженное сравнение.

### Изменено

- Постоянные кэши тел функций и строк системного каталога PostgreSQL изолированы полным SHA-256 отпечатком целевой БД и параметров сессии; внутри каждой цели используются полные SHA-256 идентификаторы reader/query.
- Размер сериализованной строки кэша системного каталога ограничен 32 МиБ, а очистка кэша сделана неблокирующей для параллельных локальных JVM-процессов. Постоянный кэш поддерживается только на локальной файловой системе; NFS и другие сетевые файловые системы не поддерживаются.
- Загрузка проекта и чтение из PostgreSQL стали существенно быстрее: строки каталога читаются параллельно и в упакованном виде, тела функций запрашиваются только когда их содержимое действительно отличается, а поиски, которые раньше просматривали коллекции целиком, работают по индексу.
- Сравнению нужно меньше памяти: на самом крупном из замеренных проектов минимальная рабочая куча снизилась с 3328 МБ до 2560 МБ, а пик живых объектов — с 2783 МБ до 1700 МБ, потому что три вещи, которые удерживались весь прогон и никем не читались, теперь освобождаются.
- Пакетный запуск строит дерево различий и оба графа зависимостей один раз на весь прогон, а не заново на каждый выход; пустая пачка не строит ничего. Все генерируемые скрипты при этом побайтово те же.
- Частичная выгрузка теперь сохраняет только те пути, которых она касается, вместо копирования всего проекта, поэтому выгрузка нескольких объектов из большого проекта выполняется почти мгновенно.
- `ON ONLY` записывается только там, где индекс действительно присоединяется к секциям, а не для каждого индекса секционированной таблицы.
- Значение параметра хранения теперь записывается так, как его называет база, — в кавычках, независимо от того, какая сторона его объявила, поэтому проект и база, написавшие один параметр по-разному, описывают один и тот же объект.
- Миграция, меняющая запрос материализованного представления ClickHouse, теперь несёт запрос ровно в том виде, в каком он написан в проекте, а не в том, который сравнение использует внутри себя.
- Колонки, скрытые правилом игнорирования, теперь не попадают в файлы проекта, которые записываются при применении состояния базы к проекту, — не только для PostgreSQL, но и для MS SQL и ClickHouse. Скрипта миграции это не касается: скрытая колонка не участвует ни в одном сравнении, поэтому для неё ничего не добавляется, не удаляется и не изменяется, а то, что скрипт всё-таки пишет, он пишет из проекта — ровно так, как проект это объявляет.

### Исправлено

- Повреждённые записи постоянного кэша строк системного каталога PostgreSQL с чрезмерно большими вложенными массивами теперь отклоняются до выделения памяти, что предотвращает непропорциональное потребление памяти в куче.
- Исправлены миграции зависимых последовательностей PostgreSQL: сначала меняется владелец таблицы, затем владелец последовательности и её права; при переименовании используется новое имя, а переживающие пересоздание колонки или таблицы последовательности временно отвязываются и привязываются обратно без потери OID и текущего значения. Фактические владельцы проверяются как до, так и после отвязки, а при фильтрации обязательной операции генерация завершается до вывода некорректного частичного скрипта.
- Исправлены четыре миграции, которые сервер отвергал: внешний ключ на удаляемом уникальном индексе теперь удаляется и создаётся вместе с ним; внешний ключ сопоставляется с ключом, на который ссылается, по составу колонок, а не по их порядку, — так же, как это делает сервер; цикл представления и функции разрывается; `ALTER TYPE ... ADD VALUE` выносится перед транзакцией, потому что PostgreSQL не даёт использовать значение перечисления в той же транзакции, где оно добавлено.
- Анализ PostgreSQL защищён от неполных `SELECT`-деревьев после восстановления парсера; исправлен анализ `ROWS FROM` без явного псевдонима, включая сигнатуры нескольких функций, псевдонимы выходных столбцов и `WITH ORDINALITY`.
- Строковые литералы PostgreSQL `regtype` теперь разбираются грамматикой типов данных: устранены ложные предупреждения для многословных, массивных и параметризованных типов и сохранены зависимости для квалифицированных пользовательских типов.
- Выражение, записанное иначе, но означающее то же самое, больше не считается изменением. Пробелы, регистр и написание типа больше не важны для предикатов и выражений колонок индекса, ограничений `CHECK` на таблице и на домене, предикатов `EXCLUDE`, выражений расширенной статистики, условий `WHEN` триггера, условий правил, значений по умолчанию колонки и домена, выражений `USING` и `WITH CHECK` политик доступа, а также для выражений движка, параметров словаря и фильтров политик ClickHouse.
- Объект с конструкцией, которую инструмент не смог прочитать, больше не сравнивается так, будто в нём нет ничего: два объекта с разными нечитаемыми конструкциями выходили равными, и разница между ними не доходила до миграции.
- Закрыты шесть источников разницы, которую никакая миграция не могла устранить. Объект показывался изменённым на каждом прогоне, а записанный для него оператор ничего не менял в каталоге — на первичном ключе и на уникальном ограничении это пересобирало индекс каждый раз, а представление с явным списком колонок дропалось и создавалось заново вместе со всем, что от него зависит. Это параметры хранения, метод доступа индекса, граница секции, `TO PUBLIC` у политики, режим хранения колонки и список колонок представления. Оба написания теперь приводятся к тому, которое хранит база.
- Команды `ALTER`, которые грамматика и так принимала, теперь доходят до модели, а не отбрасываются: формы `ALTER TABLE` для колонок, ограничений, идентификаторов, генерируемых колонок и свойств таблицы, а также `RENAME CONSTRAINT`, `SET DISTRIBUTED BY`, `RESET (...)` и формы включения-выключения триггеров и правил; `ALTER DOMAIN` со значением по умолчанию, с `NOT NULL` и с ограничением; тело `ALTER SEQUENCE`; варианты `ALTER TYPE`; формы свойств `ALTER INDEX`, `ALTER VIEW` и `ALTER MATERIALIZED VIEW`. Файл проекта с такой командой больше не приводит к миграции, отменяющей то, что этот же файл объявил.
- Ещё два оператора, которые грамматика и так принимала, доходят до модели: `ALTER TABLE ... ATTACH/DETACH PARTITION` и форма `LIKE` в определении колонок. Секция, записанная так, как её пишет дамп, — сама таблица и следом `ATTACH`, — загружалась как обычная таблица без границы и без родителя, и сравнение предлагало отсоединить живую секцию при двух одинаковых состояниях; таблица, определённая через `LIKE`, доходила до модели вообще без колонок, и сравнение писало `DROP COLUMN` на каждую.
- Ограничение или индекс, который не удалось прочитать целиком, больше не воспроизводится как некорректный SQL: ограничение с пустым списком колонок и первичный ключ, использующий существующий индекс, теперь записываются корректно, а такой первичный ключ больше не пропадает из модели.
- Нечитаемое выражение больше не превращается в неверную команду миграции: правило больше не становится `DO NOTHING`, ограничение `CHECK` не становится `CHECK (null)`, триггер не теряет условие `WHEN`, ограничение `EXCLUDE` не теряет своё выражение, значение настройки функции не становится `null`, а представление с нечитаемым запросом больше не прерывает загрузку.
- Колонка, добавляемая вместе с ограничением `NOT NULL` с пометкой `NOT VALID`, больше не теряет это ограничение. Это единственная форма not-null, которую нельзя записать внутри определения колонки, а больше её не писал никто, поэтому миграция оставляла базу без ограничения, которое объявляет проект, и возвращал его только второй прогон — тот, которому уже не нужно было добавлять колонку.
- Перенос данных при пересоздании таблицы больше не ломается на колонке, имя которой требует кавычек: вставка, поиск последовательности и блок перезапуска экранируют имя везде, где его называют.
- `ALTER TABLE ... ENABLE/DISABLE TRIGGER`, записанный для секции, теперь берёт имя триггера в кавычки. Без кавычек имя с заглавной буквой называло несуществующий триггер и оператор падал, а имя, совпадающее с ключевым словом самого оператора, молча отвечало за все триггеры таблицы вместо одного названного.
- Неудачная частичная выгрузка оставляет проект восстановимым: откат продолжается после пути, который не удалось вернуть, сохранённая копия не удаляется, а её расположение сообщается; символические ссылки восстанавливаются как ссылки, а не как копии их целей; корневой каталог, созданный самой выгрузкой, удаляется.
- Сбой при загрузке теперь сообщается, а не превращается в пустой или частичный результат, выглядящий как успешная загрузка.
- Нечитаемый список игнорирования или список зависимостей теперь прерывает операцию, а не приравнивается к пустому. Нечитаемый фильтр — это не то же самое, что отсутствие фильтра, и от этой разницы зависит, какие объекты попадают в сравнение и что окажется в скрипте.
- Исправлена обработка отложенности в PostgreSQL: `INITIALLY DEFERRED` подразумевает `DEFERRABLE`, триггер-ограничение читает оба слова своей отложенности, а внешний ключ добавляет свою команду `ALTER CONSTRAINT` в миграцию один раз, а не дважды.
- Исправлено сравнение внешнего ключа, не учитывавшее признаки, которые уже входят в его собственную идентичность: изменённый внешний ключ снова попадает в скрипт миграции.
- Исправлены `ALTER SEQUENCE ... SET LOGGED` для последовательности, объявленной схемой, удаление владельца через `OWNED BY NONE` и запись `SET DEFAULT` тем же путём, что и значение по умолчанию, объявленное внутри команды.
- Исправлены копия представления ClickHouse, разделявшая колонки с оригиналом, и смена движка представления, оставлявшая его идентичность устаревшей.
- Нечитаемое тело функции теперь сообщается с указанием того файла, из которого оно взято, а не постороннего.
- Исправлено падение при применении специфичного для Greenplum описания секционирования к таблице, которая так не секционирована.
- Генерация скрипта по выбранным объектам больше не прерывается, когда в выборке есть место, которое существует только в одной из двух баз, — потомок таблицы или схемы, которого на другой стороне никогда не было, — а выборка с двумя одноимёнными объектами в разных схемах теперь даёт скрипт для обоих.
- Игнорируемые объекты больше не показываются в дереве различий, а колонка, на которую по-прежнему ссылается остальная база, больше не скрывается.
...
- Исправлены ошибки, приводившие к падению тестов в среде Windows.
...
- Добавлен поиск файлов .pgcodekeeperignore и .pgcodekeeperignoreschema при чтении проекта.

### Изменено

- Переработана структура проекта: выделены слои `api`, `base` и модули реализаций для PostgreSQL, MS SQL и ClickHouse для облегчения расширения функциональности. API адаптировано под новую структуру.
- Обновлен год лицензии.
- Повышена версия библиотеки testcontainers.
...
- Удалена логика проверки стиля кода.
- Удалена логика сбора данных из БД с учетом ограничений внешних ключей (FK).
...
[Новое]: https://github.com/pgcodekeeper/pgcodekeeper-core/compare/v15.3.0...HEAD
