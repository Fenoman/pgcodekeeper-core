...
public final class PgOperatorsReader extends PgAbstractSearchPathJdbcReader {
    ...
    protected void processResult(ResultSet res, ISchema schema) throws SQLException {
        ...
        if (leftArgType > 0) {
            PgJdbcType leftType = loader.getCachedTypeByOid(leftArgType);
        ...
        if (rightArgType > 0) {
            PgJdbcType rightType = loader.getCachedTypeByOid(rightArgType);
            oper.setRightArg(rightType.getFullName());
            rightType.addTypeDepcy(oper);
        }

        long resultType = res.getLong("result");
        PgJdbcType returnsType = loader.getCachedTypeByOid(resultType);
