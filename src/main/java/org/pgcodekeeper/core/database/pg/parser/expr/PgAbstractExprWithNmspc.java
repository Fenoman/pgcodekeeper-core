...
public abstract class PgAbstractExprWithNmspc<T extends ParserRuleContext> extends PgAbstractExpr {
    ...
    protected PgAbstractExprWithNmspc(PgAbstractExpr parent) {
        super(parent);
    }
    ...
    protected void analyzeCte(With_clauseContext with) {
        for (With_queryContext withQuery : with.with_query()) {
            ...
            if (withSelect != null) {
                pairs = new PgSelect(this).analyze(new PgSelectStmt(withSelect), withQuery);
            } else if ((delete = data.delete_stmt_for_psql()) != null) {
                pairs = new PgDelete(this).analyze(delete);
            } else if ((insert = data.insert_stmt_for_psql()) != null) {
                pairs = new PgInsert(this).analyze(insert);
            } else if ((update = data.update_stmt_for_psql()) != null) {
                pairs = new PgUpdate(this).analyze(update);
            } else if ((merge = data.merge_stmt_for_psql()) != null) {
                pairs = new PgMerge(this).analyze(merge);
            } else {
    ...
    protected List<ModPair<String, String>> analyzeReturningSelectList(
        ...
        return select.sublist(ctx.select_list().select_sublist(), new PgValueExpr(select));
    }
