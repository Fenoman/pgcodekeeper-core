...
public class PgFunctionsReader extends PgAbstractSearchPathJdbcReader {
    ...
    private void fillAggregate(PgAggregate aggregate, ResultSet res) throws SQLException {
        ...
        PgJdbcType sType = loader.getCachedTypeByOid(res.getLong("stype"));
        ...
        if (mstype != 0) {
            PgJdbcType mSType = loader.getCachedTypeByOid(mstype);
    ...
    private List<Pair<String, ObjectReference>> fillArguments(PgAbstractFunction f,
        ...
        Long[] argTypeOids = PgJdbcUtils.getColArray(res, "proallargtypes", true);
        Long[] argTypes = argTypeOids != null ? argTypeOids : PgJdbcUtils.getColArray(res, "argtypes");
        ...
        for (int i = 0; argTypes.length > i; i++) {
            ...
            PgJdbcType returnType = loader.getCachedTypeByOid(argTypes[i]);
            ...
            PgJdbcType argJdbcType = loader.getCachedTypeByOid(argTypes[i]);
        ...
        if (DbObjType.FUNCTION == f.getStatementType() || DbObjType.AGGREGATE == f.getStatementType()) {
            ...
            if (!sb.isEmpty()) {
                ...
                PgJdbcType returnType = loader.getCachedTypeByOid(res.getLong("prorettype"));
