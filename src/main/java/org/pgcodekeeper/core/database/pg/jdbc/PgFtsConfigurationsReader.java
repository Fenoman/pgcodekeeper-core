...
public final class PgFtsConfigurationsReader extends PgAbstractSearchPathJdbcReader {
    ...
    protected void processResult(ResultSet res, ISchema schema) throws SQLException {
        ...
        String[] fragments = PgJdbcUtils.getColArray(res, "tokennames", true);

        if (fragments != null) {
            Map<String, List<String>> dictMap = new HashMap<>();
            String[] dictSchemas = PgJdbcUtils.getColArray(res, "dictschemas");
            String[] dictionaries = PgJdbcUtils.getColArray(res, "dictnames");
